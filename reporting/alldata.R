...
alldata <- function(year_month) {
  ...
  time_spent_by_user <- content_sessionlogs %>% group_by(user_id) %>% summarize(total_hours = sum(time_spent)/3600)
  
  # get the number of distinct days a user logeed in using the start_timestamp date only
  logins_by_user <- content_sessionlogs %>% distinct(user_id,start_date_only) %>% group_by(user_id) %>% summarize(total_logins = n())
  
  # get the total number of completed exercises and videos between month start and month end
  completed_ex_vid_count <- content_sessionlogs %>% filter(progress >= 0.99) %>% group_by(user_id,kind) %>% summarize(count = n())
  ...
  time_by_channel <- content_sessionlogs %>% group_by(user_id,channel_id) %>% summarise(total_time = sum(time_spent)/3600) %>% spread(channel_id, total_time)
