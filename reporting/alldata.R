...
alldata <- function(year_month) {
  ...
  rpt <- complete_summary %>% select(user_id,first_name,last_name,username,name.y,total_logins,total_hours,total_exercises,total_videos,month_end,exercises_attempted,last_active_date) %>% rename(group = name.y, id = user_id) %>% mutate(centre = rep(device_name), month_active = ifelse(total_hours>0, 1, 0))
