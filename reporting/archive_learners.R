...
archives <- function(dates, sessionlogs, summarylogs, topics, device_name, include_coach_content = FALSE) {
  ...
  write.csv(
    rpt,
    file = generate_filename("archive_learners_", year_month, device_name),
    col.names = FALSE,
    row.names = FALSE,
    na = "0"
  )
