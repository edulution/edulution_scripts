...
check_content_logs_in_curr_month <- function(year_month, month_start, month_end) {
    ...
    num_logs <- nrow(
      content_sessionlogs %>%
        filter(start_timestamp >= month_start,
...
if (nrow(learners_and_groups) == 0) {
  ...
  learners_and_groups$group <-
    rep('ungrouped', nrow(learners_and_groups))
} else{
...
if (nrow(content_sessionlogs) == 0) {
  ...
  write.csv(
    report,
    file = generate_filename("users_", Sys.Date()) ,
    col.names = FALSE,
    row.names = FALSE,
    na = "0"
  )
  ...
  content_sessionlogs <-
    content_sessionlogs %>% mutate(start_date_only = strftime(start_timestamp, "%Y-%m-%d"))
}
