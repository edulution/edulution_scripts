...
#get users
users_query <- dbSendQuery(conn,"SELECT * FROM securesync_facilityuser")
  #filter out deleted users and coaches, then select only columns needed for joins later in script
users <- dbFetch(users_query) %>% filter(deleted == 0 && is_teacher == 0) %>% select(id,username,first_name,last_name,group_id,facility_id)
